...
def analyze_time_series_features(series: pd.Series) -> dict:
    ...
    return {'length': len(series), 'mean': series.mean(), 'std': series.std(), 'trend': 'increasing' if series.iloc[-1] > series.iloc[0] else 'decreasing', 'volatility': series.pct_change().std()}
