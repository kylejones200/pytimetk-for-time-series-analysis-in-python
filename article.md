...
`pytimetk` is a time series analysis and
forecasting in Python inspired by the R package `timetk`.
...
Feature Engineering is really easy in `pytimetk` --- including more advanced concepts like Fast Fourier
...
We can use `pytimetk` to aggregate the
...
`pytimetk` connects the first and last
points in a straight line.


`pytimetk` is based on Pandas so we can
...
`pytimetk` doesn't do ARIMA or SARIMA. I
...
### So what?
I liked `pytimetk` more than I thought I
...
[GitHub](https://github.com/kylejones200/time_series/blob/main/pytimekt.ipynb).
